...
<html lang="fr">
<head>
	<title>DoMini - Tension des piles</title>
		<meta charset="UTF-8">
		    ...
		    $(function ()
		    {
		        ...
		        $.each(names, function (i, name)
		        {
							$.get('./csv/temp_' + name + '.csv', function (csv, state, xhr)
							{
								...
								// parse the CSV data
								var tension = [], header, comment = /^#/, axe, seriename, seriecolor;
								$.each(csv.split('\n'), function(i, line){
									if (!comment.test(line)) {
										if (!header) {
											...
											tension.push([x, parseFloat(point[5])]); 	            
										}
									}
								});
				...
				function createChart()
				{
					chart = new Highcharts.StockChart(
					{
						...
						// Premier axe temperatures
						{
							...
							opposite : false,
							lineWidth: 1,
							title: { text: 'Tensiions des piles'},
			...
			<div class="container">
				<div class="navbar">
				  <div class="navbar-inner">
					<a class="brand" href="#">Tension piles modules</a>
					<ul class="nav">
					</ul>
				  </div>
				</div>				
